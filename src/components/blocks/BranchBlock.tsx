    ...
    <div className="measure relative flex flex-col gap-3 sm:flex-row sm:items-stretch sm:gap-4">
      {options.map((option, index) => (
        <div key={option.label} className="flex flex-1 flex-col sm:contents">
          {index > 0 && <OrDivider />}
          <div className="flex-1 rounded-[var(--radius-sm)] border border-[var(--border-hair)] bg-[var(--bg-base)]/60 p-3.5">
...
function OrDivider() {
  return (
    <div
      aria-hidden
      className="flex items-center justify-center gap-3 py-0.5 sm:w-px sm:flex-col sm:self-stretch sm:py-0"
    >
      <span className="h-px flex-1 bg-[var(--border-hair)] sm:h-auto sm:w-px sm:flex-1" />
      <span className="t-numeral rounded-full border border-[var(--border-hair)] bg-[var(--bg-overlay)] px-2 py-1 text-[var(--text-secondary)]">
        OR
      </span>
      <span className="h-px flex-1 bg-[var(--border-hair)] sm:h-auto sm:w-px sm:flex-1" />
    </div>
